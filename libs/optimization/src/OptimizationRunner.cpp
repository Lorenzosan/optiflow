...
OptimizationDiagnostics dispatch_diagnostics(const std::vector<core::DispatchStep>& dispatch,
    ...
    for (const core::DispatchStep& step : dispatch) {
        ...
        diagnostics.wait_steps += (!turbines && !pumps && !spills) ? 1U : 0U;
        diagnostics.export_energy_mwh += step.net_power > 0.0
                                             ? step.net_power * time_step_hours
                                             : 0.0;
...
OptimizationResult OptimizationRunner::run(const core::ScenarioBundle& bundle) const {
    ...
    const solver::BellmanSolver bellman_solver(state_grid, action_grid, model, solver_parameters);
    ...
    const solver::ForwardSimulator simulator(state_grid, action_grid, model, solver_parameters);
