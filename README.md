...
OptiFlow is an end-to-end pumped-storage hydro dispatch optimization application. A C++ Bellman dynamic-programming solver determines generation, pumping, spilling, and storage decisions from electricity-price and natural-inflow forecasts.

The solver is exposed through a FastAPI service with PostgreSQL persistence, a dependency-free browser interface, automated C++, Python, and JavaScript tests, generated Doxygen documentation, and Docker Compose deployment. The project is intended to demonstrate numerical modelling, software architecture, validation, and communication of optimization results in one reproducible application.

The upper reservoir is the storage inventory. Electricity is consumed to pump water upward and generated by releasing water through the turbine.

## What is implemented

* Explicit scenario, price, and inflow CSV inputs.
...
At the committed resolution, the deterministic result exports `135 MWh electric`, finishes at `0 MWh hydraulic`, and reports `€202.50` cumulative model reward. The solver generates during two inflow intervals to preserve enough storage capacity for the pulse, then prioritizes the later `€2/MWh` block.
...
PostgreSQL is the database server. It stores relational records for scenarios, optimization runs, run status and timing, error information, artifact paths, and compact run-summary values such as energy, cashflow, runtime, and final storage. PostgreSQL data is persisted in the Docker volume `optiflow-postgres-data`.

SQLAlchemy is the Python database-access layer used by FastAPI; it is not a database. SQLAlchemy maps Python model classes to PostgreSQL tables, creates SQL queries from Python operations, manages request-scoped sessions and transactions, and represents relationships between scenarios, runs, and summaries. The `psycopg` driver carries those SQL operations between SQLAlchemy and PostgreSQL. Alembic applies versioned schema migrations when tables or columns change.

```text
FastAPI application
  -> SQLAlchemy object-relational mapping and sessions
...
Persistence is deliberately hybrid:

* PostgreSQL stores scenario metadata, run metadata and status, summary metrics, and managed artifact paths.
* Docker volumes store uploaded scenario CSV files and detailed `dispatch.csv` trajectories.
* Detailed dispatch data is kept as a CSV artifact rather than expanded into thousands of relational rows.

A typical run is persisted as follows:

1. FastAPI creates a PostgreSQL run record with status `running`.
2. FastAPI launches the compiled C++ solver as a child process.
3. The solver writes the detailed `dispatch.csv` artifact.
4. FastAPI updates the run to `succeeded` or `failed`.
5. FastAPI stores compact summary values through SQLAlchemy.
6. The browser reads run history and summaries through the API and fetches `dispatch.csv` only when detailed data is needed.
...
### Preparing price and inflow files

Price and natural-inflow forecasts use two separate UTF-8 CSV files with identical timestamps. Each timestamp denotes the start of one optimization interval.

Prices:

```csv
timestamp_utc,price
2027-01-01T00:00:00Z,42.50
2027-01-01T01:00:00Z,37.25
2027-01-01T02:00:00Z,-5.00
```
...
2027-01-01T00:00:00Z,3.2
2027-01-01T01:00:00Z,3.0
2027-01-01T02:00:00Z,2.8
```

Requirements:

* Use the exact headers `timestamp_utc,price` and `timestamp_utc,natural_inflow`.
* Use canonical UTC timestamps in `YYYY-MM-DDTHH:MM:SSZ` form. Offsets such as `+00:00` are not accepted.
* Keep timestamps strictly increasing and equally spaced.
* Use exactly the same timestamps and number of data rows in both files.
* Provide at least two timestamped rows when using the browser editor so it can derive the interval length. The command-line solver accepts a single interval when `time_step_hours` is supplied by the scenario file.
* Use finite numeric values and `.` as the decimal separator.
* Express price in `€/MWh`; positive, zero, and negative prices are supported.
* Express natural inflow as nonnegative `MW hydraulic`.
* Do not provide physical water flow or reservoir volume directly. Convert plant-specific water quantities and hydraulic head into the constant-head hydraulic-energy representation before optimization.

For an hourly scenario, consecutive timestamps differ by one hour. For a 15-minute scenario, they differ by 15 minutes. The browser editor derives `time_step_hours` from this spacing and writes it into the generated scenario CSV. The command-line solver checks that the spacing equals the scenario's `time_step_hours`. The dispatch retains `time_index` and adds the authoritative `timestamp_utc`.
